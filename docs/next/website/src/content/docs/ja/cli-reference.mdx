...
`machine availability` は次のいずれか 1 形式です。`--local-session <name>` を 1 回以上、`--all-local-sessions`、または `--no-local-sessions`。`--local-session` は繰り返し、または `--local-session=name` で指定します。これらの形式は混ぜません。`--local-session` の名前はローカルセッション名と同じ規則です。ASCII の英数字、`.`、`_`、`-`、1 から 64 バイト、`.` と `..` は不可。まだ存在しない名前も使えます。並べ替えと重複削除の前に最大 64 件です。`--all-local-sessions` は制限を省略します。`--no-local-sessions` は空の一覧を保存します。
