...
`machine add` はリモートのデフォルトセッションを使います。名前付きセッションを使う場合だけ `--remote-session <name>` を追加します。スクリプトでは `machine list` に `--json` を追加できます。ローカルセッションごとの利用可否は[ローカルセッションの利用可否](/ja/docs/connecting-machines/#ローカルセッションの利用可否)を参照してください。

`machine availability` では、フラグの前にプロファイル ID を置きます。形式は次のいずれか 1 つです。`--local-session <name>` を 1 回以上、`--all-local-sessions`、または `--no-local-sessions`。`--local-session` は繰り返し、または `--local-session=name` で指定します。これらの形式は混ぜません。`--local-session` の名前はローカルセッション名と同じ規則です。ASCII の英数字、`.`、`_`、`-`、1 から 64 バイト、`.` と `..` は不可。まだ存在しない名前も使えます。並べ替えと重複削除の前に最大 64 件です。`--all-local-sessions` は制限を省略します。`--no-local-sessions` は空の一覧を保存します。
...
変更は、開いているローカルの Herdr クライアントに通常 1 秒以内に自動反映されます。そのクライアントのローカルセッションで許可された追加または有効化済みマシンにはバックグラウンドで接続し、名前の変更では再接続しません。ローカルで許可された無効化済みの行は表示されたまま接続しません。削除または無効化すると、そのマシンだけが切断され、リモートセッションは動き続けます。表示中のマシンを削除すると Local に戻ります。Local に接続できない場合は、再接続するまで利用不可と表示します。各プロファイルは、不透明な ID、ラベル、SSH 接続先、明示的なリモートセッション名、有効状態、任意の `local_sessions` 一覧をクライアントの状態ファイルに保存します。パスワード、秘密鍵、その他の SSH 認証情報は保存しません。

`machine list` は生のカタログを読みます。制限されたプロファイルも隠しません。テキスト列は id、label、target、session、enabled または disabled、availability（`all`、`none`、または許可するローカルセッション名）、available または unavailable、selected または unselected です。`--json` は `id`、`label`、`target`、`session`、`enabled`、`local_sessions`、`available`、`selected` です。`available` と `selected` は現在のローカルセッション向けです。`HERDR_SESSION` があればその値、なければ `default`。このローカルセッションは SSH ホスト上の保存済みリモートセッションではありません。`machine list --json` では、制限がないとき `local_sessions` は `null`、どのローカルセッションでも使えないときは `[]`、制限があるときは名前の配列です。
...
有効なプロファイル ID または一意のラベル（大文字・小文字を区別）を指定します。任意の SSH ホスト名は使えません。その生の一致のあと、現在のローカルセッションでも利用できる必要があります。無効化済みプロファイルは先に使用エラーになります。対象外、または不正な継承 `HERDR_SESSION` は SSH の前に使用エラー 2 です。`--session` や `--remote` との併用はエラーです。保存済みのリモートセッションを使います。省略時のローカル動作は変わりません。明示的な `herdr --remote` は別経路で、生の有効な接続先とリモートセッションの一致を使います。
