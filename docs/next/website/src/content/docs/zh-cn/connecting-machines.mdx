...
本地会话在设置了 `HERDR_SESSION` 时用该值，否则为 `default`。`sandbox` 这类命名会话是本地的。保存的 `session` 字段是 SSH 主机上的远程 Herdr 会话。
