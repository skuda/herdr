...
移除或禁用正在查看的机器会返回 Local。如果 Local 不可用，Herdr 会显示该状态并重试连接，而不是选择其他远程机器。有本本地会话允许的已启用保存机器时，即使 Local 失败或重启，客户端仍可继续使用。

本地允许的已禁用配置会以 Disabled 留在侧边栏，不会开始连接。本本地会话不允许的配置会从客户端目录中省略。
...
每个本地会话把所选机器存在 `endpoint-selections/<session>.json`。仅 `default` 在缺少作用域文件时可能读取较旧的 `endpoint-selection.json`。命名会话不会导入该旧文件。在 UI 中选择机器会写入作用域文件。自动回退、恢复和重连会保留磁盘上的上次显式选择，重新允许时不会抢走焦点。删除命名本地会话不会移除这些共享客户端文件；用同一名称重建会复用它们。
...
- **运行中的保存机器文件错误：** 目录文件无法读取或内容无效时，当前连接保持不变。Herdr 会显示通知并保留最新的有效观测。后续错误不会丢弃尚未应用的有效重新加载。有效更改在画面交接之间应用。监视大约每秒读取一次。存在尚未应用的有效重新加载时，会暂停新的连接尝试。
