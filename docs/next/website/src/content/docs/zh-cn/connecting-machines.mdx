...
运行 `herdr` 打开 UI。如果本地客户端已经打开，该客户端本地会话允许的新增或已启用机器通常会在一秒内出现并在后台连接，不改变当前选择。切换机器期间的配置变化会在切换完成后应用。设置结束后，远程服务器仍会运行。

新配置在设置可用性之前不受限制。已打开的客户端可以在这段时间内连接。如果不想在添加时立即连接，请在添加并限制期间关闭相关的本地客户端。
...
移除或禁用正在查看的机器会返回 Local。如果 Local 不可用，Herdr 会显示该状态并重试连接，而不是选择其他远程机器。有当前本地会话允许的已启用保存机器时，即使 Local 失败或重启，客户端仍可继续使用。

本地允许的已禁用配置会以 Disabled 留在侧边栏，不会开始连接。当前本地会话不允许的配置会从客户端目录中省略。

## 本地会话可用性

可用性是本地配置，不是安全边界。它限制哪些本地 Herdr 会话可以使用已保存的 SSH 机器，不会改变 SSH 主机上的远程会话。

本地会话在设置了 `HERDR_SESSION` 时用该值，否则为 `default`。`tradingdroid` 这类命名会话是本地的。保存的 `session` 字段是 SSH 主机上的远程 Herdr 会话。
...
`--all-local-sessions` 省略 `local_sessions`（无限制）。`--no-local-sessions` 保存 `[]`（不允许任何本地会话）。一次或多次 `--local-session` 保存允许列表。名称与本地会话名相同，也可以指向尚未存在的会话。排序和去重前最多 64 个。

`machine list` 和 `machine list --json` 显示全部原始配置。`available` 和 `selected` 针对当前本地会话。在 `machine list --json` 中，无限制时 `local_sessions` 为 `null`，不允许任何本地会话时为 `[]`，有限制时为名称数组。见 [CLI 参考](/zh-cn/docs/cli-reference/#已保存的-ssh-机器)。

`--machine` 在完成原始已启用配置匹配后，使用同一当前本地会话。不符合条件或继承的 `HERDR_SESSION` 无效时，会在 SSH 之前以用法错误 2 失败。显式的 `herdr --remote <target>` 不受此限：即使配置在本地被限制，仍使用原始的已启用目标和远程会话匹配。

无效的继承本地会话加上 socket 覆盖时，只启动 Local。不会加载已保存机器，不会启动目录监视，也不会写入作用域选择文件。现有通知面会显示验证错误。普通 socket API 命令的覆盖行为不变。

每个本地会话把所选机器存在 `endpoint-selections/session-<会话名字节的小写十六进制>.json`。例如：`default` 为 `session-64656661756c74.json`；`Default` 为 `session-44656661756c74.json`；`tradingdroid` 为 `session-74726164696e6764726f6964.json`。仅精确小写名称 `default` 在缺少作用域文件时可能读取较旧的 `endpoint-selection.json`。命名会话（包括 `Default`）不会导入该旧文件，也不会使用目录中的嵌入选择。在 UI 中选择机器会写入作用域文件。自动回退、恢复和重连会保留磁盘上的上次显式选择，重新允许时不会抢走焦点。删除命名本地会话不会移除这些共享客户端文件；用同一名称重建会复用它们。

写入共享目录时，无限制会省略 `local_sessions`。较旧的严格读取方会拒绝包含该字段的目录，包括空列表。降级前请对受限制配置运行 `--all-local-sessions`。写入还会清除全局无效的嵌入所选配置；读取不会改写文件。
...
- **启动时的保存机器文件错误：** 目录文件无法读取或内容无效时，会在没有已保存机器的情况下启动，并显示通知。socket 可用时仍会连接 Local。
- **运行中的保存机器文件错误：** 目录读取错误本身不会改变连接。已经观测到的有效重新加载仍可能在画面交接之间应用。Herdr 会显示通知并保留最新的有效观测。后续错误不会丢弃尚未应用的有效重新加载。监视大约每秒读取一次。存在尚未应用的有效重新加载时，会暂停新的连接尝试。
...
配置保存不透明 ID、标签、SSH 目标、明确的远程会话名、启用状态，以及可选的 `local_sessions` 列表。Herdr 不会将密码、私钥、智能体票据或 SSH 控制套接字存入机器目录。认证仍由 OpenSSH 处理。
