...
`machine availability` 只取一种形式：一次或多次 `--local-session <name>`、`--all-local-sessions`，或 `--no-local-sessions`。可重复 `--local-session`，或使用 `--local-session=name`。不要混用这些形式。`--local-session` 名称与本地会话名相同：ASCII 字母、数字、`.`、`_`、`-`；1 到 64 字节；不能是 `.` 或 `..`。名称不必已经存在。排序和去重前最多 64 个。`--all-local-sessions` 省略限制。`--no-local-sessions` 保存空列表。
