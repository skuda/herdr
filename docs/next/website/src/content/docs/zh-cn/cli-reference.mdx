...
`machine add` 默认使用远程默认会话。只有需要命名会话时才添加 `--remote-session <name>`。`machine list` 支持可选的 `--json` 供脚本使用。本地会话可用性见[本地会话可用性](/zh-cn/docs/connecting-machines/#本地会话可用性)。

`machine availability` 要求配置 ID 写在标志之前。之后只取一种形式：一次或多次 `--local-session <name>`、`--all-local-sessions`，或 `--no-local-sessions`。可重复 `--local-session`，或使用 `--local-session=name`。不要混用这些形式。`--local-session` 名称与本地会话名相同：ASCII 字母、数字、`.`、`_`、`-`；1 到 64 字节；不能是 `.` 或 `..`。名称不必已经存在。排序和去重前最多 64 个。`--all-local-sessions` 省略限制。`--no-local-sessions` 保存空列表。
...
更改会自动应用到已打开的本地 Herdr 客户端，通常在一秒内生效。该客户端本地会话允许的新增或已启用机器会在后台连接；重命名不会重新连接。本地允许的已禁用行仍可见，但不会连接。移除或禁用只会断开该机器，远程会话仍会继续运行。移除正在查看的机器会返回 Local；如果 Local 不可用，则显示不可用状态，直到重新连接。每份配置在客户端状态文件中保存一个不透明 ID、标签、SSH 目标、明确的远程会话名、启用状态，以及可选的 `local_sessions` 列表。Herdr 不保存密码、私钥或其他 SSH 凭据。

`machine list` 读取原始目录，不会隐藏受限制的配置。文本列为 id、label、target、session、enabled 或 disabled、availability（`all`、`none` 或允许的本地会话名）、available 或 unavailable、selected 或 unselected。`--json` 使用 `id`、`label`、`target`、`session`、`enabled`、`local_sessions`、`available`、`selected`。`available` 和 `selected` 针对当前本地会话：设置了 `HERDR_SESSION` 时用该值，否则为 `default`。该本地会话不是 SSH 主机上保存的远程会话。在 `machine list --json` 中，无限制时 `local_sessions` 为 `null`，不允许任何本地会话时为 `[]`，有限制时为名称数组。
...
选择器必须是已启用的配置 ID 或唯一标签（区分大小写），不能是任意 SSH 主机名。完成该原始匹配后，配置还必须在当前本地会话中可用。已禁用配置会先返回用法错误。不符合条件或继承的 `HERDR_SESSION` 无效时，会在 SSH 之前以用法错误 2 失败。不能与 `--session` 或 `--remote` 混用。命令使用保存的远程会话。省略前缀时，本地行为保持不变。显式的 `herdr --remote` 是独立路径，仍使用原始的已启用目标和远程会话匹配。
