...
The local session is `HERDR_SESSION` when that variable is set, otherwise `default`. Named sessions such as `sandbox` are local. The saved `session` field is the remote Herdr session on the SSH host.
