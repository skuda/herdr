...
Run `herdr` to open the UI. If a local client is already open, added and enabled machines that are allowed in that client's local session normally appear within a second and connect in the background without changing your selection. An in-progress machine switch finishes before profile changes are applied. The remote server keeps running after setup exits.

A new profile is unrestricted until you set availability. Open clients can connect in that window. If you do not want that initial connection, close the relevant local clients while you add and restrict the machine.
...
Removing or disabling the machine you are viewing returns you to Local. If Local is unavailable, Herdr shows that and retries its connection instead of selecting a different remote machine. With enabled saved machines that are allowed in this local session, the client can remain usable even if Local fails or restarts.

A locally allowed disabled profile stays in the sidebar as Disabled and does not start a connection. A profile that is not allowed in this local session is omitted from the client catalog.

## Local-session availability

Availability is local configuration, not a security boundary. It limits which local Herdr sessions can use a saved SSH machine. It does not change the remote session on the SSH host.

The local session is `HERDR_SESSION` when that variable is set, otherwise `default`. Named sessions such as `tradingdroid` are local. The saved `session` field is the remote Herdr session on the SSH host.

```bash
herdr machine availability <profile-id> --local-session default
herdr machine availability <profile-id> --local-session default --local-session tradingdroid
herdr machine availability <profile-id> --all-local-sessions
herdr machine availability <profile-id> --no-local-sessions
```

`--all-local-sessions` omits `local_sessions` (unrestricted). `--no-local-sessions` stores `[]` (no local session). One or more `--local-session` flags store an allowlist. Names use the same rules as local session names and may name a session that does not exist yet. At most 64 names are accepted before sort and dedup.

`machine list` and `machine list --json` show every raw profile. `available` and `selected` are for the current local session. In `machine list --json`, `local_sessions` is `null` when unrestricted, `[]` when allowed in no local session, and an array of names when restricted. See [CLI reference](/docs/cli-reference/#saved-ssh-machines).

`--machine` uses that same current local session after the raw enabled-profile match. An ineligible machine or invalid inherited `HERDR_SESSION` fails with usage error 2 before SSH. Explicit `herdr --remote <target>` is exempt: it still uses a raw enabled target and remote-session match even when the profile is restricted locally.

An invalid inherited local session with a socket override starts Local-only. Saved machines are not loaded, the catalog watcher does not start, and scoped selection files are not written. The existing notice surface shows the validation error. Plain socket API commands keep their existing override behavior.

Each local session stores its selected machine in `endpoint-selections/session-<lowercase hex of the session-name bytes>.json`. Examples: `default` is `session-64656661756c74.json`; `Default` is `session-44656661756c74.json`; `tradingdroid` is `session-74726164696e6764726f6964.json`. For the exact lowercase name `default` only, a missing scoped file may read the older `endpoint-selection.json`. Named sessions, including `Default`, do not import that legacy file or the catalog's embedded selection. Selecting a machine in the UI writes the scoped file. Automatic fallback, recovery, and reconnect keep the last explicit choice on disk and do not steal focus when a machine is reallowed. Deleting a named local session does not remove these shared client files; recreating the same name reuses them.

Writes to the shared catalog omit `local_sessions` when unrestricted. Older strict readers reject a catalog that includes the field, including an empty list. Before downgrade, run `--all-local-sessions` on restricted profiles. Writes also clear a globally invalid embedded selected profile; a read does not rewrite the file.
...
- **Saved-machine file error at startup:** An unreadable or invalid catalog starts with no saved machines and a notice. Local still attaches when its socket is available.
- **Saved-machine file error while running:** A catalog read error does not itself change connections. A valid reload already observed may still apply between surface handoffs. Herdr shows a notice and keeps the latest valid observation. A later error does not drop a pending valid reload. The watcher reads about once per second. A pending valid reload pauses new connection attempts until that observation is applied.
...
Saved profiles contain an opaque ID, label, SSH target, explicit remote session, enabled state, and optional `local_sessions` list. Herdr does not store passwords, private keys, agent tickets, or SSH control sockets in the catalog. Authentication stays with OpenSSH.
