...
Each local session stores its selected machine in `endpoint-selections/<session>.json`. For `default` only, a missing scoped file may read the older `endpoint-selection.json`. Named sessions do not import that legacy file. Selecting a machine in the UI writes the scoped file. Automatic fallback, recovery, and reconnect keep the last explicit choice on disk and do not steal focus when a machine is reallowed. Deleting a named local session does not remove these shared client files; recreating the same name reuses them.
...
- **Saved-machine file error while running:** An unreadable or invalid catalog leaves current connections unchanged. Herdr shows a notice and keeps the latest valid observation. A later error does not drop a pending valid reload. Valid changes apply between surface handoffs. The watcher reads about once per second. A pending valid reload pauses new connection attempts until that observation is applied.
