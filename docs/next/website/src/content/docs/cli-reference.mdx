...
`machine availability` takes one form: one or more `--local-session <name>` flags, `--all-local-sessions`, or `--no-local-sessions`. Repeat `--local-session` or use `--local-session=name`. Do not mix those forms. `--local-session` names use the same rules as local session names: ASCII letters, numbers, `.`, `_`, and `-`; 1 to 64 bytes; not `.` or `..`. A name does not have to exist yet. At most 64 names are accepted before sort and dedup. `--all-local-sessions` omits the restriction. `--no-local-sessions` stores an empty list.
