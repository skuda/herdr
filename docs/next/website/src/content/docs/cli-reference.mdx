...
By default, `machine add` uses the remote default session. Add `--remote-session <name>` only for a named session. `machine list` accepts optional `--json` for scripts. See [Connecting machines](/docs/connecting-machines/#local-session-availability) for local-session availability.

`machine availability` requires the profile ID first. It then takes one form: one or more `--local-session <name>` flags, `--all-local-sessions`, or `--no-local-sessions`. Repeat `--local-session` or use `--local-session=name`. Do not mix those forms. `--local-session` names use the same rules as local session names: ASCII letters, numbers, `.`, `_`, and `-`; 1 to 64 bytes; not `.` or `..`. A name does not have to exist yet. At most 64 names are accepted before sort and dedup. `--all-local-sessions` omits the restriction. `--no-local-sessions` stores an empty list.
...
Changes apply automatically to open local Herdr clients, normally within a second. Added or enabled machines connect in the background when they are allowed in that client's local session; renaming does not reconnect. A locally allowed disabled row stays visible and does not connect. Removing or disabling disconnects only that machine and leaves its remote sessions running. Removing the machine you are viewing returns to Local, or shows Local as unavailable until it reconnects. Each profile stores an opaque ID, label, SSH target, explicit remote session, enabled state, and optional `local_sessions` list in client state. Herdr does not store passwords, private keys, or other SSH credentials.

`machine list` reads the raw catalog. It does not hide restricted profiles. Text columns are id, label, target, session, enabled or disabled, availability (`all`, `none`, or the allowed local session names), available or unavailable, and selected or unselected. `--json` uses `id`, `label`, `target`, `session`, `enabled`, `local_sessions`, `available`, and `selected`. `available` and `selected` are for the current local session: `HERDR_SESSION` when set, otherwise `default`. That local session is not the saved remote session on the SSH host. In `machine list --json`, `local_sessions` is `null` when unrestricted, `[]` when allowed in no local session, and an array of names when restricted.
...
The selector must match an enabled saved profile ID or a unique, case-sensitive label, not an arbitrary SSH hostname. After that raw match, the profile must also be available in the current local session. A disabled profile fails first with a usage error. An ineligible or invalid inherited `HERDR_SESSION` fails with usage error 2 before SSH. Combining `--machine` with `--session` or `--remote` is an error. The saved remote session is used. Without the prefix, commands retain their existing local session/socket behavior. Explicit `herdr --remote` is separate and still uses a raw enabled target and remote-session match.
